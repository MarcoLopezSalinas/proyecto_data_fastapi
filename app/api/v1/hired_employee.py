...
def insert_hired_employees(employees: List[HiredEmployeeCreate], db: Session = Depends(get_db)):
    """
    Inserta de 1 a 5000 empleados contratados en una sola petición
    """
    if len(employees) == 0:
        raise HTTPException(status_code=400, detail="La lista está vacía")
    if len(employees) > 5000:
        raise HTTPException(status_code=400, detail="No puedes insertar más de 5000 registros por petición")
